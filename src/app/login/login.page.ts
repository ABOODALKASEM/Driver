  ...
  async signIn() {
    try {
      ...
      this.authY.onAuthStateChanged(async (user) => {
        if (user) {
          alert('User Authenticated: '+  user.uid);
          const isRider = await this.avatar.getUserType(user.uid);
          alert('User Type: ' +  isRider);
          this.overlay.hideLoader();
          if (isRider) {
            this.overlay.showAlert('Error', 'Drivers cannot log in as riders.');
      ...
      //if (e.code === 'auth/invalid-app-credential' || e.code === 'auth/too-many-requests') {
        this.CountryCode = '+234';
        this.numberT = '+234';
        const defaultNumbers = ['7089930308', '7089930308'];
